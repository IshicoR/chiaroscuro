...
pub fn view<Message: 'static>(sample: Option<TelemetrySample>) -> Element<'static, Message> {
    ...
    container(
        column![
            ...
            rear,
            container(
                text("Carcass I / M / O · hot pressure (IBT)")
                    .size(10)
...
fn tyre_heading<Message: 'static>(value: String, size: f32) -> Element<'static, Message> {
    container(text(value).size(size).font(typography::MONO_SEMIBOLD))
...
fn temperature_segment<Message: 'static>(
    ...
    container(
        column![
            text(label).size(9).font(typography::MONO_SEMIBOLD),
            text(value).size(10).font(typography::MONO_SEMIBOLD),
        ]
