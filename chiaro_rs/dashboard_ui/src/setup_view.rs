...
fn summary_content<'a>(
    ...
    let statuses = status_badges(data);
    ...
    content
        .push(container(details).padding(4).width(Length::Fill))
        .into()
}

fn status_badges<'a>(data: &'a SetupViewData) -> Vec<Element<'a, DashboardMessage>> {
...
fn section_content(section: &SetupSection) -> Element<'_, DashboardMessage> {
...
fn setup_rows_view(rows: Vec<&SetupRow>) -> Element<'_, DashboardMessage> {
...
fn corner_comparison_view(comparison: CornerComparison) -> Element<'static, DashboardMessage> {
...
fn corner_table_header(columns: &[SetupCorner]) -> Element<'static, DashboardMessage> {
...
fn corner_table_label_cell(label: String, header: bool) -> Element<'static, DashboardMessage> {
...
fn setup_group_row<'a>(label: &'a str, depth: usize) -> Element<'a, DashboardMessage> {
    container(
        text(label)
            .size(TABLE_BODY_SIZE)
            .font(typography::SANS_SEMIBOLD),
...
fn status_content(data: &SetupViewData) -> Element<'_, DashboardMessage> {
    match &data.status {
